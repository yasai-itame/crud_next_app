...
const EditMemberModal: React.FC<MemberType> = React.memo(({ id, name, email, age, createdAt, editSubmit, setEditData }) => {
  const nameValueChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    if (typeof setEditData != 'undefined') {
      setEditData((prev) => ({
        ...prev,
  ...
  const emailValueChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (typeof setEditData != 'undefined') {
      setEditData((prev) => ({
        ...prev,
  ...
  return (
    <div id="hs-edit-member-modal" className="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto pointer-events-none">
      <div className="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-4xl sm:w-full m-3 h-[calc(100%-3.5rem)] sm:mx-auto">
        <div className="max-h-full overflow-hidden flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto dark:bg-neutral-900 dark:border-neutral-800 dark:shadow-neutral-700/70">
          <div className="flex justify-between items-center py-3 px-4 border-b dark:border-neutral-800">
            ...
            <button type="button" className="flex justify-center items-center size-7 text-sm font-semibold rounded-lg border border-transparent text-gray-800 hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none dark:text-white dark:border-transparent dark:hover:bg-neutral-700" data-hs-overlay="#hs-edit-member-modal">
          ...
          <div className="mt-5 p-4 mx-4 my-4 relative z-10 bg-white border rounded-xl sm:mt-10 md:p-10 dark:bg-neutral-900 dark:border-neutral-700">
            <div className="mb-4 sm:mb-8">
              {
                <>
                  <InputBlock key={`id-name-${id}`} id={`id-name-${id}`} type="text" mb="mb-3" title="Name" value={name} onInputChange={(e) => nameValueChange(e)} />
                  <InputBlock key={`id-email-${id}`} id={`id-email-${id}`} type="text" mb="mb-3" title="Email" value={email} onInputChange={(e) => emailValueChange(e)} />
                  <div className="flex justify-end">
                    <Button text="Submit" color="blue" onClick={editSubmit} />
                  </div>
                </>
              }
            </div>
